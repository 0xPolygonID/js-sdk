...
describe('Adapter', () => {
    it('Test adapter for v0.0.1 HTTP schema', async () => {
        const issuerDid = DID.parse('did:polygonid:polygon:amoy:2qQ68JkRcf3xyDFsGSWU5QqxbKpzM75quxS628JgvJ');
        const userId = DID.idFromDID(DID.parse('did:polygonid:polygon:amoy:2qQ68JkRcf3xyDFsGSWU5QqxbKpzM75quxS628JgvJ'));
    ...
    it('Test adapter for v0.0.1 IPFS schema', async () => {
        ...
        const userId = DID.idFromDID(DID.parse('did:polygonid:polygon:amoy:2qZYiH9CFMoo6oTjSEot3qzkHFHhjLRLKp8yfwCYng'));
        const adapter = new Adapter(RPC_URL, issuerDid, {
