...
describe('tests packageManager', () => {
  it('tests package manager with zkp  packer', async () => {
    ...
    const mockProvingMethod = new ProvingMethodGroth16Auth(
      new ProvingMethodAlg('groth16-mock', 'auth')
    );
    const mockAuthInputsHandler = new AuthDataPrepareHandlerFunc(mockPrepareAuthInputs);

    proving.registerProvingMethod(mockProvingMethod.methodAlg, () => {
    ...
    const b = await pm.pack(p.mediaType(), payloadBytes, { senderID: id });
    const basicMessg = await pm.unpack(b);

    const token = await Token.parse(bytesToString(b));
