...
class ProofService {
    ...
    async generateMTPV2Inputs(preparedCredential, identifier, proofReq, params) {
        ...
        const { query, vp } = await this.toCircuitsQuery(proofReq.query, preparedCredential.credential, preparedCredential.credentialCoreClaim);
    ...
    async prepareMerklizedQuery(query, credential) {
        ...
        if (parsedQuery.fieldName) {
            path = await js_jsonld_merklization_1.Path.getContextPathKey(encoding_1.byteDecoder.decode(ldContext), credential.type[1], parsedQuery.fieldName, this._ldOptions);
        }
    ...
    async prepareNonMerklizedQuery(query, credential) {
        ...
        parsedQuery.query.slotIndex = await schema_processor_1.Parser.getFieldSlotIndex(parsedQuery.fieldName, query.type, ldContext);
        const { vp, mzValue, dataType } = await (0, verifiable_1.verifiablePresentationFromCred)(credential, query, parsedQuery.fieldName, this._ldOptions);
        if (parsedQuery.isSelectiveDisclosure) {
            const resultQuery = parsedQuery.query;
            resultQuery.operator = circuits_1.QueryOperators.$eq;
    ...
    async loadLdContextFromJSONSchema(jsonSchemaURL) {
