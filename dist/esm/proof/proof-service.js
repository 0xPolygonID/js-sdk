...
import { Parser, cacheLoader } from '../schema-processor';
...
export class ProofService {
    ...
    async prepareMerklizedQuery(query, credential) {
        ...
        if (parsedQuery.fieldName) {
            path = await Path.getContextPathKey(byteDecoder.decode(ldContext), credential.type[1], parsedQuery.fieldName, this._ldOptions);
        }
    ...
    async prepareNonMerklizedQuery(query, credential) {
        ...
        parsedQuery.query.slotIndex = await Parser.getFieldSlotIndex(parsedQuery.fieldName, query.type, ldContext);
        const { vp, mzValue, dataType } = await verifiablePresentationFromCred(credential, query, parsedQuery.fieldName, this._ldOptions);
        if (parsedQuery.isSelectiveDisclosure) {
            const resultQuery = parsedQuery.query;
            resultQuery.operator = QueryOperators.$eq;
