...
import { CredentialStatus, CredentialStatusType, DisplayMethod, DisplayMethodType, Iden3SparseMerkleTreeProof, VerifiableConstants, W3CCredential } from "../../../verifiable";
...
const booleanHashTrue = "18586133768512220936620570745912940619677854269274689475585506675881198879027"
const booleanHashFalse = "19014214495641488759237505126948346942972912379615652741039992445865937985820"
...
export class OnchainNonMerklizedIssuerAdapter {
    ...
    public async getCredential(issuerId: Id, credentialId: bigint): Promise<{
    ...
    public async convertOnChainInfoToW3CCredential(credentialData: INonMerklizedIssuer.CredentialDataStructOutput,
        coreClaimBigInts: bigint[],
        credentialSubjectFields: INonMerklizedIssuer.SubjectFieldStructOutput[]): Promise<W3CCredential> {
    ...
    private credentialStatus(nonce: number): CredentialStatus {
        const id = `${this._issuerDid.string()}/credentialStatus?revocationNonce=${nonce}&contractAddress=${this._chainId}:${this._contractAddress}`;
        return {
            id: id,
    ...
    private async convertCredentialSubject(
        ...
        credentialSubjectFields: INonMerklizedIssuer.SubjectFieldStructOutput[]): Promise<JsonDocumentObject> {
        ...
        for (const f of credentialSubjectFields) {
            ...
            switch (dataType) {
                ...
                case XSDNS.NonNegativeInteger, XSDNS.NonPositiveInteger, XSDNS.NegativeInteger, XSDNS.PositiveInteger: {
    ...
    private convertDisplayMethod(
        onchainDisplayMethod : INonMerklizedIssuer.DisplayMethodStructOutput): DisplayMethod|undefined {
