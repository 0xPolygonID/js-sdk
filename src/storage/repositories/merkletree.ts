import { InMemoryDB, ITreeStorage, Merkletree, str2Bytes } from '@iden3/js-merkletree';
...
export class MerkleTreeRepository implements IMerkleTreeStorage {
  ...
  mtDepth: number;
  constructor(_mtDepth: number, private treeStorage: ITreeStorage) {
  ...
  async createIdentityMerkleTrees(
    ...
    mtTypes.forEach((t) => {
      ...
      const tree = new Merkletree(this.treeStorage, true, this.mtDepth);
